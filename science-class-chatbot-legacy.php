...
class Science_Class_Sidebar_Chatbot {
    ...
    private $coins_per_objective = 3;
    private $max_coins_per_lesson = 10;
    ...
    private function get_lesson_objectives($lesson) {
        if (isset($lesson['objectives']) && is_array($lesson['objectives']) && !empty($lesson['objectives'])) {
            $clean = array();
            foreach ($lesson['objectives'] as $objective_text) {
                $text = sanitize_text_field((string) $objective_text);
                if ($text !== '') {
                    $clean[] = $text;
                }
            }
        ...
        $single = isset($lesson['objective']) ? sanitize_text_field((string) $lesson['objective']) : '';
        return $single !== '' ? array($single) : array();
    }

    private function get_lesson_coin_total_from_rewards($rewards, $lesson_slug) {
        if (!is_array($rewards) || empty($rewards)) {
            return 0;
        }

        $lesson_slug = sanitize_title((string) $lesson_slug);
        $total = 0;
        foreach ($rewards as $reward) {
            if (!is_array($reward)) {
                continue;
            }
            $reward_slug = !empty($reward['lesson_slug']) ? sanitize_title((string) $reward['lesson_slug']) : '';
            if ($reward_slug !== $lesson_slug) {
                continue;
            }
    ...
    private function extract_met_objective_indexes($chat_result, $objective_count) {
        $indexes = array();

        if (isset($chat_result['met_objective_indexes']) && is_array($chat_result['met_objective_indexes'])) {
            foreach ($chat_result['met_objective_indexes'] as $raw_idx) {
                $idx = is_numeric($raw_idx) ? (int) $raw_idx : -1;
                if ($idx >= 0 && $idx < $objective_count) {
                    $indexes[$idx] = true;
                }
            }
        }
    ...
    public function handle_send_message() {
        ...
        if ($lesson_slug === '' && !empty($selected_lesson['slug'])) {
            $lesson_slug = sanitize_title($selected_lesson['slug']);
        }
        if ($lesson_slug === '') {
            $fallback_slug_source = !empty($selected_lesson['id']) ? $selected_lesson['id'] : $selected_lesson['title'];
            $lesson_slug = sanitize_title((string) $fallback_slug_source);
        }
        $lesson_objectives = $this->get_lesson_objectives($selected_lesson);
        if (empty($lesson_objectives)) {
            wp_send_json_error(array('error' => 'Lesson has no objectives configured.'), 500);
        }

        $rewards = $this->get_user_objective_rewards($user_id);
        $completed_objective_indexes = array();
        foreach ($lesson_objectives as $idx => $objective_text) {
            $objective_key = $this->build_objective_key($lesson_slug, $objective_text);
            if (isset($rewards[$objective_key])) {
                $completed_objective_indexes[$idx] = true;
            }
        }

        $next_incomplete_idx = null;
        foreach ($lesson_objectives as $idx => $objective_text) {
            if (!isset($completed_objective_indexes[$idx])) {
                $next_incomplete_idx = $idx;
                break;
            }
        }

        $objectives_for_prompt = array();
        foreach ($lesson_objectives as $idx => $objective_text) {
            $status_label = isset($completed_objective_indexes[$idx]) ? 'completed' : 'incomplete';
            $objectives_for_prompt[] = sprintf('[%d] %s (%s)', $idx, $objective_text, $status_label);
        }
        ...
        $system_prompt .= "Current objectives:\n" . implode("\n", $objectives_for_prompt) . "\n\n";
        if ($next_incomplete_idx !== null) {
            $system_prompt .= "Current focus objective index: {$next_incomplete_idx}\n";
            $system_prompt .= "Current focus objective text: {$lesson_objectives[$next_incomplete_idx]}\n\n";
        }
        ...
        $system_prompt .= "Set met_objective_indexes to at most one index: only the current focus objective if it was demonstrated in this latest student message.\n";
        $system_prompt .= "Set objective_met=true only when all lesson objectives are completed.\n";
        $system_prompt .= "If objective_met is false, assign exactly 1 concrete task in tasks.\n";
        $system_prompt .= "Return JSON only with keys: reply (string), objective_met (boolean), met_objective_indexes (array of integers), tasks (array of short strings).\n";
        $system_prompt .= "If objective_met is true, include a brief congratulatory message and tasks must be empty.";
        ...
        $met_indexes = $this->extract_met_objective_indexes($chat_result, count($lesson_objectives));
        if ($next_incomplete_idx !== null) {
            $met_indexes = array_values(array_filter($met_indexes, function ($idx) use ($next_incomplete_idx) {
                return (int) $idx === (int) $next_incomplete_idx;
            }));
        } else {
            $met_indexes = array();
        }

        $llm_says_complete = !empty($chat_result['objective_met']);
        if ($llm_says_complete && empty($met_indexes) && $next_incomplete_idx !== null) {
            $met_indexes[] = $next_incomplete_idx;
        }

        if ($next_incomplete_idx !== null && !isset($completed_objective_indexes[$next_incomplete_idx])) {
            if ($this->objective_heuristic_met($lesson_objectives[$next_incomplete_idx], $message)) {
                $met_indexes[] = $next_incomplete_idx;
            }
        }
        $met_indexes = array_values(array_unique(array_map('intval', $met_indexes)));

        $token = '';
        $coins_awarded = 0;
        $coin_balance = $this->get_user_coin_balance($user_id);
        $lesson_coins_before = $this->get_lesson_coin_total_from_rewards($rewards, $lesson_slug);
        $remaining_lesson_cap = max(0, $this->max_coins_per_lesson - $lesson_coins_before);
        $awarded_objective_keys = array();

        foreach ($met_indexes as $idx) {
            if ($idx < 0 || $idx >= count($lesson_objectives) || isset($completed_objective_indexes[$idx])) {
                continue;
            }
            $objective_text = $lesson_objectives[$idx];
            $objective_key = $this->build_objective_key($lesson_slug, $objective_text);
            if (isset($rewards[$objective_key])) {
                $completed_objective_indexes[$idx] = true;
                continue;
            ...
            $completed_count_before = count($completed_objective_indexes);
            $is_final_objective_for_lesson = ($completed_count_before + 1) >= count($lesson_objectives);
            if ($is_final_objective_for_lesson) {
                // Final objective: top up to the lesson cap so total reaches 10.
                $coins_for_this_objective = $remaining_lesson_cap;
            } else {
                $coins_for_this_objective = min($this->coins_per_objective, $remaining_lesson_cap);
            }
            if ($coins_for_this_objective > 0) {
                if ($token === '') {
                    $token = 'YH-' . strtoupper(wp_generate_password(8, false, false));
                }
                $coins_awarded += $coins_for_this_objective;
                $remaining_lesson_cap -= $coins_for_this_objective;
            }

            $rewards[$objective_key] = array(
                'token' => $token,
                'lesson_slug' => $lesson_slug,
                'objective' => (string) $objective_text,
                'coins_awarded' => $coins_for_this_objective,
                'completed_at' => current_time('mysql'),
            );
            $completed_objective_indexes[$idx] = true;
            $awarded_objective_keys[] = $objective_key;
        }
        ...
        if (!empty($awarded_objective_keys)) {
            update_user_meta($user_id, $this->objective_rewards_meta_key, $rewards);
        }

        $objective_met = count($completed_objective_indexes) >= count($lesson_objectives);
        $tasks = isset($chat_result['tasks']) && is_array($chat_result['tasks']) ? $chat_result['tasks'] : array();
        $tasks = array_values(array_filter(array_map(function ($task) {
            return sanitize_text_field((string) $task);
        }, $tasks)));
        $tasks = array_slice($tasks, 0, 1);

        if (!$objective_met && empty($tasks)) {
            $next_idx = null;
            foreach ($lesson_objectives as $idx => $objective_text) {
                if (!isset($completed_objective_indexes[$idx])) {
                    $next_idx = $idx;
            ...
            if ($next_idx !== null) {
                $tasks[] = 'Complete this objective: ' . $lesson_objectives[$next_idx];
            }
        }
        ...
        if ($objective_met && $token === '') {
            foreach ($lesson_objectives as $objective_text) {
                $objective_key = $this->build_objective_key($lesson_slug, $objective_text);
                if (isset($rewards[$objective_key]['token']) && $rewards[$objective_key]['token'] !== '') {
                    $token = sanitize_text_field((string) $rewards[$objective_key]['token']);
                    break;
                }
            }
        }
        ...
        $wpdb->insert(
            ...
            array(
                ...
                'conversation' => wp_json_encode(array(
                    ...
                    'objective_keys' => $awarded_objective_keys,
